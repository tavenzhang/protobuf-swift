...
public class UnknownFieldSet:Hashable,Equatable {
    ...
    public func writeToCodedOutputStream(output:CodedOutputStream) throws {
        ...
        for number in sortedKeys {
            ...
            try value.writeTo(fieldNumber: number, output: output)
        }
    }
    ...
    public class func parseFromCodedInputStream(input:CodedInputStream) throws -> UnknownFieldSet {
        return try UnknownFieldSet.Builder().mergeFromCodedInputStream(input: input).build()
    }
    
    
    public class func parseFromData(data:Data) throws -> UnknownFieldSet {
        return try UnknownFieldSet.Builder().mergeFromData(data: data).build()
    }
    
    
    public class func parseFromInputStream(input:InputStream) throws -> UnknownFieldSet {
        return try UnknownFieldSet.Builder().mergeFromInputStream(input: input).build()
    }

    public class func builderWithUnknownFields(copyFrom:UnknownFieldSet) throws -> UnknownFieldSet.Builder {
        return try UnknownFieldSet.Builder().mergeUnknownFields(other: copyFrom)
    }
    ...
    public func writeAsMessageSetTo(output:CodedOutputStream) throws {
        for number in fields.keys {
            let field:Field = fields[number]!
            try field.writeAsMessageSetExtensionTo(fieldNumber: number, output:output)
        }
    }
    ...
    public class Builder {
        ...
        public func mergeFromData(data:Data) throws -> UnknownFieldSet.Builder {
            let input:CodedInputStream = CodedInputStream(data: data)
            _ = try mergeFromCodedInputStream(input: input)
            try input.checkLastTagWas(value: 0)
            return self
        }
        
        public func mergeFromInputStream(input:InputStream) throws -> UnknownFieldSet.Builder {
            throw ProtocolBuffersError.obvious("UnsupportedMethod")
        }
        public func mergeFromInputStream(input:InputStream, extensionRegistry:ExtensionRegistry) throws -> UnknownFieldSet.Builder {
            throw ProtocolBuffersError.obvious("UnsupportedMethod")
        }
        ...
        public func mergeFromCodedInputStream(input:CodedInputStream) throws -> UnknownFieldSet.Builder {
            while (true) {
                ...
                let mergeField = try mergeFieldFrom(tag: tag, input:input)
                if tag == 0 || !(mergeField)
                {
                    break
                }
